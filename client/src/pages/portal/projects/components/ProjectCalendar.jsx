        ...
        <div className="not-dark:bg-white dark:bg-linear-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
          ...
          <div className="grid grid-cols-7 gap-2">
            {daysInMonth.map((day) => {
              ...
              const isEnd =
                projectEnd &&
                format(day, "yyyy-MM-dd") === format(projectEnd, "yyyy-MM-dd");
              ...
              return (
                ...
                >
                  ...
                  {isStart && <span className="text-[10px]">Start</span>}
                  {isEnd && <span className="text-[10px]">End</span>}
                  {dayTasks.length > 0 && (
              ...
              {getTasksForDate(selectedDate).map((task) => (
                ...
                >
                  <div className="flex justify-between mb-2">
                    ...
                    <span
                      className={`flex items-center px-2 py-0.5 rounded text-xs ${typeColors[task.type]}`}
                    >
                      {task.type}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs text-zinc-600 dark:text-zinc-400">
                    ...
                    <div className="flex -space-x-1.5 overflow-hidden">
                      {(task?.assignees || []).map((a, i) => (
                        <img
                          key={a._id || a.id || i}
                          className="inline-block size-5 rounded-full ring-2 ring-white dark:ring-zinc-900 object-cover bg-zinc-200"
